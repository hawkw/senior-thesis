...
While Rust and Mnemosyne have similar goals, Rust is among the first languages with lifetime-based memory management to gain widespread adoption. Thus, it seems that there is still a need for increased diversity of languages in this design space. Rust and Mnemosyne differ along a number of axes. Rust's syntax is inspired primarily by that of C and the languages descended from it, while Mmemosyne's is drawn from the Lisp family. Many programmers find Lisp-style homoiconic expressions to be powerful, elegant, and expressive, as discussed in ~\Cref{sec:background}. Furthermore, future Mnemosyne development is planned to include features not present in Rust. For example, in a future release, Mnemosyne will enforce function purity, as in Haskell~\cite{jones2003haskell,hudak1992gentle,hudak1992report}, which will eventually be developed into a fully featured effect system in which the side effects of functions are encoded in the type system~\cite{Marino:2009:GTS:1481861.1481868}.
...
fn parse_def(&self, input: State<I>) -> ParseResult<Form<'a, U>, I> {
    ...
    let function_form // The function definition form consists of...$\label{line:funcform}$
        = self.name() // ... an identifier...
              .and(self.function()) // ...followed by a function body
              .map(|(name, fun)| // convert parse results to AST node $\label{line:map1}$
                    DefForm::Function { name: name
                                      , fun: fun });

    let top_level // The top-level def form consists of...$\label{line:toplvl}$
        = self.name() // ...an identifier...
              .and(self.type_name()) // ...and type annotation...
              .and(self.expr()) // ...followed by some expression.
              .map(|((name, ty), body)| // convert results to AST node $\label{line:map2}$
                DefForm::TopLevel { name: name
                                  , annot: ty
                                  , value: Rc::new(body) });

    // A `def` statement consists of...
    self.reserved("def") // ...the `def' keyword...$\label{line:def}$
        .or(self.reserved("define")) //...or the `define' keyword $\label{line:define}$...
        .with(function_form.or(top_level)) // ...and a definition.
        .map(Form::Define) // convert parse results to AST node $\label{line:map3}$
        .parse_state(input)
}
    \end{minted}
